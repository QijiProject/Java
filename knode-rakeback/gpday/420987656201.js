		...
		if (err) {
			...
			connection.beginTransaction(function(ex) {
				if (ex) {
					...
					var sql = 'insert into npt_ld_rktmp ('
				    sql += 'select wagersid,playerid,gamedate,betamount+progressivebet,winamount from npt_ld_wagered_detail where rrflag=0 and status=1 and gamedate<='+self.task._sdate + ')';
					...
					connection.query(sql, function(e, result) {
						if (e) {
							...
							var sql1 = 'update npt_ld_wagered_detail set rrflag=1 where wagersid<>"" and (gamedate,wagersid) in(select gamedate,wagersid from npt_ld_rktmp)';
		...
		if (err) {
			...
			connection.beginTransaction(function(ex) {
				if (ex) {
					...
					sql += 'created,updated,status) (select '+task.sid+',playerid,'+task.gpid+',' + task._intdate + ',sum(realmoneybets),sum(realmoneywins),count(*),' + dt + ',' + dt + ',1 from npt_ld_rktmp group by playerid)';
					...
					connection.query(sql, function(e, result) {
						if (e) {
							...
							connection.query('delete from npt_ld_rktmp where wagersid<>""', function(e1, result) {
								if (e1) {
									connection.rollback(function() {
								        DbMgr.closeConn(ctype, connection);
										self.emit('nextAcpGpid');
