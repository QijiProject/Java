...
WaterSXing.prototype.getWater = function(acpid, item, dt, callback) {
    var sql = 'insert into iplay_player_gp_monthly ('
    sql += 'select '+item.asmonth+','+this.gpid+',playerid,agentcode,'+dt+','+dt+',';
    sql += 'count(*),sum(betamtu),sum(betamtu+winamtu),sum(betamtu) from sxing_wagered';
    sql += ' where betstatus=1 and settled=1 and bettime>='+item.cstart+' and bettime<=' + item.cend;
    ...
    DbMgr.getConnection(acpid, 2, function(err, ctype, connection) {
        if (err) {
            ...
            logger.error('mysql getconnection on %s WaterSXing getWater %s', acpid, String(err));
            callback(new Error('sxing'), []);
        } else {
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterSXing getWater query %s', acpid, String(e));
                    callback(new Error('sxing1'), []);
                } else {
                    callback(null, rows);
                }
            });
