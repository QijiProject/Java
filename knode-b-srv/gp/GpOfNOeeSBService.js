...
util.inherits(NOeeBetSB, EventEmitter);

NOeeBetSB.prototype.start = function(gp) {
	this.gp = gp;
	var cfg = this.gp.cfg;
...
NOeeBetSB.prototype._doData = function(err, response, body, actype, obj, callback) {
...
NOeeBetSB.prototype.getUnsettled = function(o, callback) {
...
NOeeBetSB.prototype._unsettled = function(data, obj, callback) {
	...
	if (data.OpenBets && (openbets=data.OpenBets.OpenBet)) {
		...
		for (var i = 0, len = openbets.length; i < len; ++i) {
			item = openbets[i];
			if (item.MemberCode == obj.player.gpn) {
...
NOeeBetSB.prototype.checkTransfer = function(o, callback) {
		...
		if (response && response.statusCode === 200) {
			...
			self.xmlparser.parseString(data, function (err, result) {
				if (result) {
					...
					logger.info('[CheckTransfer] nOeesb dno=' + o.dno + ', c=' + code + ', s=' + status);
