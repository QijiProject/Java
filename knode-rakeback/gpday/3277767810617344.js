...
GdLd.prototype.start = function(ptag) {
	this.ptag = ptag;
};

GdLd.prototype.digestTask = function(task) {
	var self = this;
	this.task = task;
...
GdLd.prototype.getDate = function() {
...
GdLd.prototype.getIntDate = function(dt) {
	return moment(dt * 1000).tz(TIMEZONE).format(INTDATE_PATTERN);
};

// 取出流水
GdLd.prototype.waterDayStatistic = function() {
	if (!this.task) return;
	var self = this;
	this.getDate();
	DbMgr.getConnection(this.task.acpid, 2, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd doWaterDayStatistic %s', self.task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.beginTransaction(function(ex) {
				if (ex) {
					logger.error('mysql getconnection on %s gd doWaterDayStatistic action %s', self.task.acpid, String(ex));
					DbMgr.closeConn(ctype, connection);
					self.emit('nextAcpGpid');
				} else {
					var sql = 'insert into gd_ld_rktmp ('
				    sql += 'select betid,playerid,bettime,betamount,winloss from gd_ld_wagered where rrflag=0 and status=1 and bettime<='+self.task._sdate + ')';
				    sql += ' on duplicate key update bettime=values(bettime),betamount=values(betamount),winloss=values(winloss)';
					connection.query(sql, function(e, result) {
						if (e) {
							...
							var sql1 = 'update gd_ld_wagered set rrflag=1 where betid<>"" and (bettime,betid) in(select bettime,betid from gd_ld_rktmp)';
...
// 日累加
GdLd.prototype._doStatistic = function() {
	var self = this, task = this.task;
	DbMgr.getConnection(task.acpid, 2, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _doStatistic %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.beginTransaction(function(ex) {
				if (ex) {
					logger.error('mysql getconnection on %s gd _doStatistic action %s', task.acpid, String(ex));
					DbMgr.closeConn(ctype, connection);
					self.emit('nextAcpGpid');
				} else {
					var dt = moment().tz(TIMEZONE).unix();
					var sql = 'insert into iplay_member_daysum(sid,playerid,gpid,sday,bettotal,efftotal,betc,';
					sql += 'created,updated,status) (select '+task.sid+',playerid,'+task.gpid+',' + task._intdate + ',sum(betamount),sum(winloss),count(*),' + dt + ',' + dt + ',1 from gd_ld_rktmp group by playerid)';
					...
					connection.query(sql, function(e, result) {
						if (e) {
							...
							connection.query('delete from gd_ld_rktmp where betid<>""', function(e1, result) {
								if (e1) {
									connection.rollback(function() {
								        DbMgr.closeConn(ctype, connection);
										self.emit('nextAcpGpid');
...
GdLd.prototype._updateSettle = function() {
	var self = this, task = this.task;
	DbMgr.getConnection(task.acpid, 1, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _updateSettle %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.query('update iplay_member_settlements set status=1,updated=unix_timestamp() where status=0 and sid='+task.sid+' and gpid=' + task.gpid, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _updateSettle query %s', task.acpid, String(e));
        			self.emit('nextAcpGpid');
        		} else {
        			process.nextTick(function() {
        				self._calcRakeback();
...
GdLd.prototype._calcRakeback = function() {
	var self = this;
	DbMgr.getConnection(this.task.acpid, 2, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _calcRakeback %s', self.task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			...
			connection.query('select sid,playerid,gpid,sday,bettotal,efftotal,betc from iplay_member_daysum where status=1 and sid<='+self.task.sid+' and gpid='+self.task.gpid+' order by created asc limit ' + self.calcStartIdx + ',' + PAGESIZE, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _calcRakeback query %s', self.task.acpid, String(e));
        			self.emit('nextAcpGpid');
        		} else {
...
GdLd.prototype._genRb = function() {
...
GdLd.prototype._doRb = function(wsum) {
...
// 计算返水比例和预计返水额
GdLd.prototype._calcRate = function(groupid, wsum) {
...
// 添加返水审核记录
GdLd.prototype._rakebackInsert = function(rs, wsum) {
	var self = this, task = this.task, dt = moment().tz(TIMEZONE).unix();
	DbMgr.getConnection(task.acpid, 1, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _rakebackInsert %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextRb');
		} else {
			...
			connection.query(sql, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _rakebackInsert query %s', task.acpid, String(e));
        			self.emit('nextRb');
        		} else {
...
// 更新日结记录状态
GdLd.prototype._updateSumStat = function(dt, wsum) {
	var self = this, task = this.task;
	DbMgr.getConnection(task.acpid, 2, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _rakebackInsert %s', task.acpid, String(err));
...
GdLd.prototype._getPlayerGroupId = function(wsum) {
	var self = this, task = this.task;
	DbMgr.getConnection(task.acpid, 1, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _getPlayerGroupId %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.query('select groupid from iplay_member where playerid=' + wsum.playerid, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _getPlayerGroupId query %s', task.acpid, String(e));
        			self.emit('nextAcpGpid');
        		} else {
        			try {
	        			...
	        			logger.error('do rb on %s gd _getPlayerGroupId query %s %s', task.acpid, wsum.playerid, String(er));
	        			self.emit('nextAcpGpid');
	        		}
        		}
       		});
...
// 玩家层级
GdLd.prototype._getPlayerRR = function() {
	var self = this, task = this.task;
	self.groupMap = {};
	DbMgr.getConnection(task.acpid, 1, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _getPlayerRR %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.query('select groupid,gpid,rrperiod,rrlimit,rrate,stepped,stepcond from iplay_mgroup_rr where gpid=' + task.gpid, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _getPlayerRR query %s', task.acpid, String(e));
        			self.emit('nextAcpGpid');
        		} else {
        			if (rows.length == 0) {
        				self.emit('completeTask');
...
GdLd.prototype._mapGroup = function(rows) {
...
GdLd.prototype._completeTask = function() {
	var self = this, task = this.task;
	DbMgr.getConnection(task.acpid, 1, function(err, ctype, connection) {
		if (err) {
			logger.error('mysql getconnection on %s gd _completeTask %s', task.acpid, String(err));
			DbMgr.closeConn(ctype, connection);
			self.emit('nextAcpGpid');
		} else {
			connection.query('update iplay_member_settlements set status=2,updated=unix_timestamp() where status=1 and sid='+task.sid+' and gpid=' + task.gpid, function(e, rows) {
				DbMgr.closeConn(ctype, connection);
        		if (e) {
        			logger.error('mysql getconnection on %s gd _completeTask query %s', task.acpid, String(e));
        		}
        		self.emit('nextAcpGpid');
       		});
		}
...
GdLd.prototype._nextAcpGpid = function() {
	this.daysum = [];
	this.calcStartIdx = 0;
	this.calcPage = 1;
	this.groupMap = null;
