...
WaterTGPSlot.prototype.getWater = function(acpid, item, dt, callback) {
    var sql = 'insert into iplay_player_gp_monthly ('
    sql += 'select '+item.asmonth+','+this.gpid+',playerid,agentcode,'+dt+','+dt+',';
    sql += 'count(*),sum(betamt),sum(winamt),sum(betamt) from tgp_slots_wagered';
    ...
    DbMgr.getConnection(acpid, 2, function(err, ctype, connection) {
        if (err) {
            ...
            logger.error('mysql getconnection on %s WaterTGPSlot getWater %s', acpid, String(err));
            callback(new Error('WaterTGPSlot'), []);
        } else {
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterTGPSlot getWater query %s', acpid, String(e));
                    callback(new Error('WaterTGPSlot1'), []);
                } else {
                    callback(null, rows);
                }
            });
