...
WaterBBINHunter.prototype.getWater = function(acpid, item, dt, callback) {
    var sql = 'insert into iplay_player_gp_monthly ('
    sql += 'select '+item.asmonth+','+this.gpid+',playerid,agentcode,'+dt+','+dt+',';
    sql += 'count(*),sum(betamount),sum(betamount+payoff),sum(commissionable) from bbin_hunter_wagered';
    ...
    DbMgr.getConnection(acpid, 2, function(err, ctype, connection) {
        if (err) {
            ...
            logger.error('mysql getconnection on %s WaterBBINHunter getWater %s', acpid, String(err));
            callback(new Error('bbinhunter'), []);
        } else {
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterBBINHunter getWater query %s', acpid, String(e));
                    callback(new Error('bbinhunter1'), []);
                } else {
                    callback(null, rows);
                }
            });
