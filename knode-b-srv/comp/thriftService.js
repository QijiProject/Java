    ...
    this.client.pullData(t, lastup, config.appname, type, s1, s2, isim, tagid, function(err, message) {
        ...
        if (type == config.thriftKey.dbs) {
            if (s1 == 3) {
                ...
                MongoService.emit('check', _data == null ? _data : _data[0], isFirst);
            }
        } else if (type == config.thriftKey.domain) {
            DomainService.emit('check', _data, isFirst);
        } else if (type == config.thriftKey.gpaccount) {
            GpService.emit('check', _data, isFirst, isim);
