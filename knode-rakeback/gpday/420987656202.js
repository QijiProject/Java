		...
		if (err) {
			...
			connection.beginTransaction(function(ex) {
				if (ex) {
					...
					var sql = 'insert into npt_regular_rktmp ('
				    sql += 'select wagersid,playerid,gamedate,realmoneybets+progressiveshare,realmoneywins from npt_regular_wagered where rrflag=0 and status=1 and gamedate<='+self.task._sdate + ')';
					...
					connection.query(sql, function(e, result) {
						if (e) {
							...
							var sql1 = 'update npt_regular_wagered set rrflag=1 where wagersid>0 and (gamedate,wagersid) in(select gamedate,wagersid from npt_regular_rktmp)';
		...
		if (err) {
			...
			connection.beginTransaction(function(ex) {
				if (ex) {
					...
					sql += 'created,updated,status) (select '+task.sid+',playerid,'+task.gpid+',' + task._intdate + ',sum(realmoneybets),sum(realmoneywins),count(*),' + dt + ',' + dt + ',1 from npt_regular_rktmp group by playerid)';
					...
					connection.query(sql, function(e, result) {
						if (e) {
							...
							connection.query('delete from npt_regular_rktmp where wagersid>0', function(e1, result) {
								if (e1) {
									connection.rollback(function() {
								        DbMgr.closeConn(ctype, connection);
										self.emit('nextAcpGpid');
