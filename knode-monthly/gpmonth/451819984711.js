...
WaterPtR.prototype.getWater = function(acpid, item, dt, callback) {
    ...
    sql += 'count(*),sum(realmoneybets),sum(realmoneywins),sum(realmoneybets) from pt_regular_wagered';
    ...
    DbMgr.getConnection(acpid, 2, function(err, ctype, connection) {
        if (err) {
            ...
            logger.error('mysql getconnection on %s WaterPtR getWater %s', acpid, String(err));
            callback(new Error('ptregular'), []);
        } else {
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterPtR getWater query %s', acpid, String(e));
                    callback(new Error('ptregular1'), []);
                } else {
                    callback(null, rows);
                }
            });
