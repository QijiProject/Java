...
WaterPtRegular.prototype.getWater = function(callback, obj, cond) { // realmoneybets
	var sql = 'select a.playerid,sum(a.betamount) as betamt from pt_regular_wagered_detail a';
    ...
    DbMgr.getConnection(obj.acpid, 2, function(err, ctype, connection) {
        if (err) {
            logger.error('mysql getconnection on %s WaterPtRegular getWater %s', obj.acpid, String(err));
            ...
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterPtRegular getWater query %s', obj.acpid, String(e));
...
WaterPtRegular.prototype.getAgentWater = function(callback, obj, cond) {
	var sql = 'select a.agentcode,sum(a.betamount) as betamt from pt_regular_wagered_detail a';
    ...
    DbMgr.getConnection(obj.acpid, 2, function(err, ctype, connection) {
        if (err) {
            logger.error('mysql getconnection on %s WaterPtRegular getAgentWater %s', obj.acpid, String(err));
            DbMgr.closeConn(ctype, connection);
            ...
            connection.query(sql, function(e, rows) {
                DbMgr.closeConn(ctype, connection);
                if (e) {
                    logger.error('mysql getconnection on %s WaterPtRegular getAgentWater query %s', obj.acpid, String(e));
