...
App.prototype.start = function() {
	...
	RedisService.on('pmessage', function(pattern, topic, message) {
		try {
			...
			logger.error('[Knode-monthly Service] parse redis pmessage error', e);
		}
	});

	Monthly.start(this);

	logger.info('[Knode-monthly Service] monthly service start');
...
App.prototype.emitInfo = function(topic, data) {
	RedisService.exec(REDISKEY, function(err, redisClient) {
		if (err) {
			logger.error('[Knode-monthly Service] exec redis publish error', err);
		} else {
