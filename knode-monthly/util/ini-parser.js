...
function safe(val) {
	return (typeof val !== "string"
			|| val.match(/[\r\n]/)
			|| val.match(/^\[/)
			|| (val.length > 1 && val.charAt(0) === "\"" && val.slice(-1) === "\"") || val !== val
			.trim()) ? JSON.stringify(val) : val.replace(/;/g, '\\;');
}
...
exports.loadIniFileSync = function(fileName/* , charset */) {
	return exports.parse(fs.readFileSync(fileName, "utf-8"));
};

exports.saveIniFileAsync = function(fileName, ini, callback) {
	fs.writeFile(fileName, ini, function(err) {
		if (callback)
			callback(err);
	});
};